...
(function (root) {
  ...
  function lineEvidence(file, text, start, end = start, url) {
    const lines = text.split("\n");
    const lineStart = Math.max(1, start), lineEnd = Math.min(lines.length, end);
    return { file, lineStart, lineEnd, excerpt: lines.slice(lineStart - 1, lineEnd).join("\n"), ...(url ? { url } : {}) };
  }
  ...
  function tokenize(text, python = false) {
    ...
    while (i < text.length) {
      ...
      if (!python && c === "/") {
        ...
        if (!prev || ["=", "(", "[", "{", ":", ",", ";", "!", "?", "return", "yield", "=>"].includes(prev.value)) {
          i++; let bracket = false;
  ...
  function documentFacts(text, language) {
    const symbols = [], imports = [], lineAt = (offset) => text.slice(0, offset).split("\n").length;
  ...
  function build(entries, options = {}) {
    ...
    function evidence(file, text, start, end, url) {
      const item = lineEvidence(file, text, start, end, url), remaining = Math.max(0, limits.maxEvidenceBytes - evidenceBytes);
    ...
    for (const entry of prepared) {
      ...
      if (typeof entry.text === "string" && !entry.text.includes("\0") && (info.text || info.kind === "file") && !["binary", "unreadable", "symlink", "metadata-only", "excluded"].includes(entry.status)) {
        ...
        if (remaining > 0) {
          ...
          node.evidence = [evidence(entry.path, text, 1, Math.min(8, text.split("\n").length), url)]; node.evidenceStatus = "source-text";
          node.summary = `${info.language || "Text"} · ${text.split("\n").length} preview lines${truncated ? " · preview truncated" : ""}`;
          if (["python", "javascript", "typescript"].includes(info.language)) facts = codeFacts(text, info.language);
          else if (["html", "markdown"].includes(info.language)) facts = documentFacts(text, info.language);
